...
export default function MobileQuotePopup({
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center backdrop-blur-md bg-black/30">
      ...
      <div className="relative flex flex-row h-[80vh] w-full max-w-7xl bg-gradient-to-b from-black to-neutral-900 text-white rounded-xl overflow-hidden border border-gray-700">
        {/* Close Button */}
        {/* <button 
          onClick={handleClose}
          className="absolute top-6 right-6 z-10 text-3xl font-bold text-white hover:text-gray-300 transition-colors"
    ...
    >
      ×
        ...
        {/* Accordion Section */}
        <div className="w-[40%] bg-[#5b5a5a]
 p-8 overflow-y-auto">
          <div className="text-center">
            <h2 className="text-4xl font-bold">₹ {data.price}</h2>
            <p className="text-sm text-gray-100 mb-6">/sq.ft (Incl. GST)</p>

            <div className="inline-block px-4 py-2 text-sm font-semibold rounded-lg bg-[#884e4e] mb-8">
              <select
                ref={selectRef}
